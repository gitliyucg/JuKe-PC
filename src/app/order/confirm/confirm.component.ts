...
export class ConfirmComponent implements OnInit {
  ...
  constructor(
    ...
    http.get(sessionStorage['http']+'/action/shopcars/GetPCconfirm?uid='+localStorage['uid']+'&ids='+this.id,{responseType: 'text'}).subscribe(data => {
      ...
      for(var i=0;i< this.model.length;i++)
      {
        ...
        this.CarIDS.push({
          CarID: this.model[i].ID,
          PID: this.model[i].PID,
          Title: this.model[i].Title,
          Images: this.model[i].Images,
          ParamID: this.model[i].ParamID,
          Params: this.model[i].ParamName,
          Price: this.model[i].Price,
          Num: this.model[i].Num,
        })
      }
    });
